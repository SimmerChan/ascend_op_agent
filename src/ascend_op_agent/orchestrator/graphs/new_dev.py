...
def build_new_dev_graph(
    ...
    codegen_node = make_llm_node(
        ...
        task_prompt_template=(
            ...
            "{state}\n\n"
            "【必做:必须把代码落到磁盘,否则下一阶段编译会失败】\n"
            "【首选方式】用 file_write 工具落盘 —— 对每个文件调一次 file_write(path, content)。\n"
            "【如果 file_write 失败或你不确定怎么用】用 markdown 代码块输出,每块首行是路径注释:\n"
            "```cpp\n"
            "// {operator_dir}/op_kernel.cpp\n"
            "<完整 C++ 源码>\n"
            "```\n"
            "```cpp\n"
            "// {operator_dir}/op_host.cpp\n"
            "<完整 C++ 源码>\n"
            "```\n"
            "```cmake\n"
            "// {operator_dir}/CMakeLists.txt\n"
            "<CMake 内容>\n"
            "```\n"
            "```bash\n"
            "// {operator_dir}/build.sh\n"
            "#!/bin/bash\n"
            "set -e\n"
            "cmake -B build -DPKG ascend910b && cmake --build build -j 8\n"
            "```\n\n"
            "operator_dir 默认 = /tmp/e2e_ops_local/op_add (用户 task 里指定的目标路径)。\n"
            "绝对禁止:把代码直接贴成普通文本(没代码块包裹),下一阶段读不到。"
        ),
