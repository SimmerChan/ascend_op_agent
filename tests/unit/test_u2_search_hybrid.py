...
def test_search_query_filters_by_task_type(fixture):
    ...
    res = _action_search(SearchSkillsArgs(query="skill", task_type="develop"))
...
def test_search_query_filters_by_topic(fixture):
    ...
    res = _action_search(SearchSkillsArgs(query="skill", topic="tiling"))
    assert res["success"] is True
    names = sorted(d["name"] for d in res["data"])
...
def test_search_query_combined_task_type_and_topic(fixture):
    ...
    res = _action_search(SearchSkillsArgs(query="skill", task_type="develop", topic="tiling"))
    assert res["success"] is True
...
def test_search_empty_query_with_task_type(fixture):
    ...
    res = _action_search(SearchSkillsArgs(query=None, task_type="migrate"))
    assert res["success"] is True
    names = sorted(d["name"] for d in res["data"])
...
def test_search_empty_query_task_type_plus_topic(fixture):
    ...
    res = _action_search(SearchSkillsArgs(query=None, task_type="develop", topic="runtime"))
    assert res["success"] is True
    names = [d["name"] for d in res["data"]]
...
def test_search_empty_query_topic_only_goes_frontmatter(fixture):
    ...
    res = _action_search(SearchSkillsArgs(query=None, topic="cuda"))
    assert res["success"] is True
    names = [d["name"] for d in res["data"]]
...
def test_search_result_shape_has_required_keys(fixture):
    ...
    res = _action_search(SearchSkillsArgs(query="tiling", task_type="develop"))
    assert res["success"] is True
...
def test_skill_manage_search_entry_passes_query(fixture):
    ...
    res = skill_manage(action="search", search_query="tiling", search_task_type="develop")
    assert res["success"] is True
    names = [d["name"] for d in res["data"]]
    assert names == ["dev-tiling"]
...
def test_skill_manage_search_entry_passes_filters_only(fixture):
    ...
    res = skill_manage(action="search", search_task_type="migrate", search_topic="cuda")
    assert res["success"] is True
    names = [d["name"] for d in res["data"]]
    assert names == ["mig-cuda"]
