...
**要求**: Agent必须先填充模板中的所有变量值，生成完整的方案设计文档后，再展示给用户确认。不得以未填充的占位符形式展示。
...
- 执行编译命令
- 捕获编译错误
- **深度修复策略**: 尝试修复可确定性修复的编译错误，包括：
  - 语法错误和拼写错误
  - 缺失的头文件、库依赖
  - 类型不匹配和声明错误
  - 缺失的分号、括号等配对符号
- **不自动修复**: 逻辑错误、算法正确性问题（需用户确认后手动修复）
...
**验证方式**:
- 生成性能报告（含延迟、吞吐、内存占用等指标）
- 提供性能数据供用户对比业界基准（需用户在规划阶段确认基准对比对象）
- 性能达标由用户自行判断，Agent不预设pass/fail阈值
...
| 阶段 | 确认内容 | 用户操作 | 超时处理 |
|------|----------|----------|----------|
| C1: 方案设计 | 实现方案、难度预估 | yes/no/modify | 简单/中等算子: 5分钟无响应自动使用建议方案继续；复杂算子: 暂停等待用户确认 |
